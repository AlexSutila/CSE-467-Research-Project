...
class variational_autoencoder:
    ...
    def __init__(self, df: DataFrame, latent_dim: int, batch_size: int, hidden_nodes: int):

        encoder_input, decoder_input = Input(shape=(df.shape[1],)), Input(shape=(latent_dim,))
        
        # Create the corresponding models to represent the encoder and decoder
        encoder_model = self.__create_encoder(encoder_input, latent_dim, hidden_nodes)
        decoder_model = self.__create_decoder(decoder_input, latent_dim, df.shape[1], hidden_nodes)
        vae_output = decoder_model(encoder_model(encoder_input))

        self.vae = Model(encoder_input, vae_output)


    def __create_encoder(self, encoder_input: Input, latent_dim: int, hidden_nodes: int):
    ...
    def __create_decoder(self, decoder_input: Input, latent_dim: int, df_cols: tuple, hidden_nodes: int) -> Model:
        ...
        return Model(decoder_input, Dense(df_cols, activation='linear')(batch_normalization_2))
